...
def plot_and_save_history(history, plot_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, 'training_history_VGG.png'))
    plt.close()

def train_vgg(new_directory_path, directory):
    ...
    model_checkpoint = ModelCheckpoint(os.path.join(model_dir, 'best_VGG_model.keras'), save_best_only=True)
    ...
    model.save(model_dir + '/VGG_final_model.h5')
