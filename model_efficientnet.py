...
def create_model():
    # Load EfficientNetB0 with pretrained ImageNet weights, excluding the top layers
...
def plot_and_save_history(history, plot_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, 'training_history_EfficientNet.png'))
    plt.close()

def train_efficientnet(new_directory_path, directory):
    ...
    model_checkpoint = ModelCheckpoint(os.path.join(model_dir, 'best_EfficientNetB0_model.keras'), save_best_only=True)
    ...
    model.save(model_dir + '/EfficientNetB0_final_model.h5')
