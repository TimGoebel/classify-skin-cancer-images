...
def plot_and_save_history(history, plot_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, 'training_history_InceptionV3.png'))
    plt.close()

def train_inceptionv3(new_directory_path, directory):
    ...
    model_checkpoint = ModelCheckpoint(os.path.join(model_dir, 'best_InceptionV3_model.keras'), save_best_only=True)
    ...
    model.save(model_dir + '/InceptionV3_final_model.h5')
